...
class Mc extends View
{
    ...
    private function parseInlcude($matches)
    {
        ...
        if ($matches['2']) {
            ...
            Response::error("include参数有误，得不到设置的模版，参数[{$matches['2']}]，解析模版路径[{$includeFile}]");
        }
        return '';
    }
    ...
    private function parseBlock($matches)
    {
        ...
        if (empty($attribute['name']) || $attribute['name'] === false) {
            return "<?php echo \\ptcms\\Block::getdata('{$attribute['method']}',array(" . implode(',', $var) . "));?>";
        } else {
            $name = '$' . $attribute['name'];
            return "<?php $name=\\ptcms\\Block::getdata('{$attribute['method']}',array(" . implode(',', $var) . "));?>";
        }
    }
