...
class tree {
    ...
    public function getSonList($pid = 0, $field = '*', $where = array(), $limitlevel = 3, $order = 'ordernum asc', $level = 1) {
        $where[$this->pidkey] = $pid;
        $data = $this->model->field($field)->where($where)->order($order)->select();
        if ($data === null) return array();
        $list = array();
        foreach ($data as $v) {
            ...
            if ($level < $limitlevel) {
                $v['sons'] = $this->getSonList($v[$this->model->getPk()], $field, $where, $limitlevel, $order, $level + 1);
            }
            $list[] = $v;
        }
        return $list;
    ...
    public function getAuthList($pid = 0, $field = '*', $where = array(), $order = 'ordernum asc', $level = 1) {
        $where[$this->pidkey] = $pid;
        $data = $this->model->field($field)->where($where)->order($order)->select();
        if ($data === null) return array();
        $list = array();
        foreach ($data as $v) {
            $v['level'] = $level;
            if ($level < 4) {
                $son=$this->getAuthList($v[$this->model->getPk()], $field, $where, $order, $level + 1);
                if ($level == 3) {
