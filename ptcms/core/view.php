...
class View
{
	...
	protected function getTplFile($tpl, $module = null, $theme = null)
	{
		if (substr($tpl, 0, 1) === '/') { //绝对目录 可以设置模版
			$tplfile = PT_ROOT . $tpl;
			$tmpl = $this->tplpath . "/default/common";
		} else {
			if ($tpl == '') {
			...
			//设置了模版 那么就去设置的模版目录 否则 就去application下面的对应目录
			if ($theme) {
				$tplfile = $this->tplpath . "/{$theme}/{$module}/{$tpl}.html";
				if (!is_file($tplfile) && $theme !== 'default') { //没有找到的模版默认使用default匹配一次
					$tplfile = $this->tplpath . "/default/{$module}/{$tpl}.html";
					$theme = 'default';
				}
				$tmpl = $this->tplpath . "/{$theme}/common";
			} else {
				$tplfile = APP_PATH . "/{$module}/view/{$tpl}.html";
				$tmpl = APP_PATH . "/{$module}/view/";
			}
		}
		$realtpl = str_replace('\\', '/', realpath($tplfile));
		if (!$realtpl) {
			halt("模版{$tpl}不存在:" . $tplfile . '<br />' . $tmpl);
		}
		defined('__TMPL__') || define('__TMPL__', PT_DIR . str_replace(PT_ROOT, '', $tmpl));
		return $realtpl;
	}
	...
	private function replaceRuninfo($content)
	{
		if (strpos($content, '__RUNINFO__') !== false) {
			...
			if (C('is_gen_html')) C('gen_html_replace', $runtimeinfo);
		}
		return $content;
	}
	...
	public function compile($content)
	{
		...
		// 解析微件
		$content = preg_replace_callback('/' . $left . 'widget((?:\s+\w+\s*\=\s*(?:"[^"]*"|\'[^\']*\'|#\w+|\$\w+(?:(?:\[(?:[^\[\]]+|(?R))*\])*|(?:\.\w+)*)|[^"\'\s]+?))+)\s*' . $right . '/i', array('self', 'parseWidget'), $content);
		// 解析循环
	...
	// 解析微件
	private function parseWidget($matches)
	{
		...
		if (empty($attribute['_etc']['template'])) {
			return "<?php $name=W('{$attribute['method']}',array(" . implode(',', $var) . "));?>";
		} else {
			return "<?php echo W('{$attribute['method']}',array(" . implode(',', $var) . "));?>";
		}
	}
