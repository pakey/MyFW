...
class Model
{
	...
	{
		$params = array_change_key_case(C('DB_MYSQL'));
	...
	{
		if ($this->master) {
			return $this->master[array_rand($this->master)];
		}
		$driverclass = 'Driver_Model_' . $this->driver;
		foreach ($this->config['master'] as $k => $v) {
			$v = array_change_key_case($v);
			$this->master[$k] = new $driverclass($v);
		}
		if ($this->singleton)
	...
	{
		if ($this->slave) {
			return $this->slave[array_rand($this->slave)];
		}
		$driverclass = 'Driver_Model_' . $this->driver;
		foreach ($this->config['slave'] as $k => $v) {
			$v = array_change_key_case($v);
			$this->slave[$k] = new $driverclass($v);
		}
		if ($this->singleton)
	...
	public function getTableField($tablename = '')
	{
		...
		if (false && APP_DEBUG === false && is_file($cachefile))
	...
	protected function parseTable()
	{
		if (empty($this->parts['table'])) {
			...
			return str_replace('!@#_', $this->config['prefix'], $this->parts['table']);
		}
	}
